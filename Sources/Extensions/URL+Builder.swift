...
internal extension URL {
    ...
    static func / (lhs: URL, rhs: String) -> URL? {
        ...
        // Ensure absolute path when a host exists (URLComponents requires leading "/" for valid URLs with host)
        if comps.host != nil, !comps.path.hasPrefix("/") {
            comps.path = "/" + comps.path
        }

        // Append the rhs component (rhs is guaranteed to not start with "/")
